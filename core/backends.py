...
class OllamaBackend(LLMBackend):
    def __init__(self, model: str | None = None, base_url: str | None = None):
        ...
        # Set a timeout (default 300s) to avoid hanging indefinitely on concurrent loads
        self._client = ollama.Client(host=self.base_url, timeout=config.LLM_TIMEOUT)
...
class VLLMBackend(LLMBackend):
    ...
    def __init__(
        ...
        # Set a timeout (default 300s) to avoid hanging indefinitely on network issues
        self._client = OpenAI(base_url=self.base_url, api_key=self.api_key, timeout=config.LLM_TIMEOUT)
