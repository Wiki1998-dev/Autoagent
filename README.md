...
AutoAgent is designed for automotive engineers, quality assurance teams, and compliance officers who require a repeatable, traceable investigation workflow. Given a failure scenario identifier (for example, `SC-042`), the system executes the following steps:
...
Every stage writes its inputs, outputs, and agent decisions to an append-only structured audit log, so any report generated by AutoAgent can be traced back to the exact evidence and reasoning that produced it.
...
Before installing AutoAgent, ensure the following are available on the host system:
...
AutoAgent is configured exclusively through environment variables, read from a `.env` file in the project root. Refer to `.env.example` for all available options. The principal variables are listed below.
...
**Embedding model changes.** If `EMBED_MODEL` is changed after an initial ingestion, the ChromaDB collection metadata will not match and AutoAgent will raise a `RuntimeError` at startup. Run `python main.py --ingest` to rebuild the index with the new model.
